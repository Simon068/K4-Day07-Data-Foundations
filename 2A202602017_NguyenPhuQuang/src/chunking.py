...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        # Base case: no separators left or text fits in chunk_size
        ...
        if separator == "":
            # Empty string separator means split character by character -> force chunk
            chunks = []
            for i in range(0, len(current_text), self.chunk_size):
                piece = current_text[i : i + self.chunk_size]
                if piece:
        ...
        current_merge = ""
        for part in parts:
            candidate = (current_merge + separator + part) if current_merge else part
            if len(candidate) <= self.chunk_size:
                current_merge = candidate
            else:
                # Flush what we have
                if current_merge:
                    chunks.append(current_merge)
                    current_merge = ""
