...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        raw_parts = [part for part in current_text.split(separator) if part != ""]
        if len(raw_parts) <= 1:
            # Separator doesn't occur here; fall back to the next one.
            return self._split(current_text, rest)

        # Recurse on any part still too big, so every piece fits chunk_size.
        pieces: list[str] = []
        ...
        # Greedily re-merge adjacent pieces (up to chunk_size) to keep context together.
        merged: list[str] = []
        current = ""
        for piece in pieces:
